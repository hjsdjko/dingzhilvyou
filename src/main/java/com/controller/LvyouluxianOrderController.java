...
public class LvyouluxianOrderController {
    ...
    */
    ...
    public R update(@RequestBody LvyouluxianOrderEntity lvyouluxianOrder, HttpServletRequest request){
...
//            lvyouluxianOrder.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<LvyouluxianOrderEntity> queryWrapper = new EntityWrapper<LvyouluxianOrderEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        LvyouluxianOrderEntity lvyouluxianOrderEntity = lvyouluxianOrderService.selectOne(queryWrapper);
        if(lvyouluxianOrderEntity==null){
    ...
    */
    ...
    public R add(@RequestBody LvyouluxianOrderEntity lvyouluxianOrder, HttpServletRequest request){
        ...
        if("用户".equals(role)){
            ...
            else if((lvyouluxianEntity.getLvyouluxianKucunNumber() -lvyouluxianOrder.getBuyNumber())<0){
            ...
            lvyouluxianOrder.setLvyouluxianOrderTypes(3); //设置订单状态为已支付
            ...
            lvyouluxianOrder.setInsertTime(new Date());
            lvyouluxianOrder.setCreateTime(new Date());
                lvyouluxianEntity.setLvyouluxianKucunNumber( lvyouluxianEntity.getLvyouluxianKucunNumber() -lvyouluxianOrder.getBuyNumber());
                lvyouluxianService.updateById(lvyouluxianEntity);
    ...
    public R add(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        String lvyouluxianOrderUuidNumber = String.valueOf(new Date().getTime());

        //获取当前登录用户的id
        Integer userId = (Integer) request.getSession().getAttribute("userId");

        Integer lvyouluxianOrderPaymentTypes = Integer.valueOf(String.valueOf(params.get("lvyouluxianOrderPaymentTypes")));//支付类型

        String data = String.valueOf(params.get("lvyouluxians"));
        JSONArray jsonArray = JSON.parseArray(data);
        List<Map> lvyouluxians = JSON.parseObject(jsonArray.toString(), List.class);

        //获取当前登录用户的个人信息
        YonghuEntity yonghuEntity = yonghuService.selectById(userId);

        //当前订单表
        List<LvyouluxianOrderEntity> lvyouluxianOrderList = new ArrayList<>();
        //商品表
        List<LvyouluxianEntity> lvyouluxianList = new ArrayList<>();

        BigDecimal zhekou = new BigDecimal(1.0);

        //循环取出需要的数据
        for (Map<String, Object> map : lvyouluxians) {
           //取值
            Integer lvyouluxianId = Integer.valueOf(String.valueOf(map.get("lvyouluxianId")));//商品id
            Integer buyNumber = Integer.valueOf(String.valueOf(map.get("buyNumber")));//购买数量
            LvyouluxianEntity lvyouluxianEntity = lvyouluxianService.selectById(lvyouluxianId);//购买的商品
            String id = String.valueOf(map.get("id"));

            //判断商品的库存是否足够
            if(lvyouluxianEntity.getLvyouluxianKucunNumber() < buyNumber){
                //商品库存不足直接返回
                return R.error(lvyouluxianEntity.getLvyouluxianName()+"的库存不足");
            }else{
                //商品库存充足就减库存
                lvyouluxianEntity.setLvyouluxianKucunNumber(lvyouluxianEntity.getLvyouluxianKucunNumber() - buyNumber);
            }
            ...
            //赋值订单信息
            lvyouluxianOrderEntity.setLvyouluxianOrderUuidNumber(lvyouluxianOrderUuidNumber);//订单号
            lvyouluxianOrderEntity.setLvyouluxianId(lvyouluxianId);//旅游路线
            lvyouluxianOrderEntity.setYonghuId(userId);//用户
            lvyouluxianOrderEntity.setBuyNumber(buyNumber);//购买数量 ？？？？？？
            lvyouluxianOrderEntity.setLvyouluxianOrderTypes(3);//订单类型
            lvyouluxianOrderEntity.setLvyouluxianOrderPaymentTypes(lvyouluxianOrderPaymentTypes);//支付类型
            lvyouluxianOrderEntity.setInsertTime(new Date());//订单创建时间
            lvyouluxianOrderEntity.setCreateTime(new Date());//创建时间

            //判断是什么支付方式 1代表余额 2代表积分
            if(lvyouluxianOrderPaymentTypes == 1){//余额支付
                //计算金额
                Double money = new BigDecimal(lvyouluxianEntity.getLvyouluxianNewMoney()).multiply(new BigDecimal(buyNumber)).multiply(zhekou).doubleValue();
                ...
                if(yonghuEntity.getNewMoney() - money <0 ){
                    ...
                    lvyouluxianOrderEntity.setLvyouluxianOrderTruePrice(money);
        ...
        lvyouluxianOrderService.insertBatch(lvyouluxianOrderList);
        lvyouluxianService.updateBatchById(lvyouluxianList);
        yonghuService.updateById(yonghuEntity);
        return R.ok();
    }
    ...
    */
    ...
    public R refund(Integer id, HttpServletRequest request){
        ...
        if("用户".equals(role)){
            ...
            Integer lvyouluxianOrderPaymentTypes = lvyouluxianOrder.getLvyouluxianOrderPaymentTypes();
            ...
            lvyouluxianOrder.setLvyouluxianOrderTypes(2);//设置订单状态为退款
            lvyouluxianOrderService.updateById(lvyouluxianOrder);//根据id更新
            yonghuService.updateById(yonghuEntity);//更新用户信息
            lvyouluxianService.updateById(lvyouluxianEntity);//更新订单中物品的信息
            return R.ok();
        }else{
